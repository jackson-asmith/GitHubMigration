...
$InvokeRepoValidation = {
    ...
    # Native-command failures are surfaced through $LASTEXITCODE checks below;
    # do not let PowerShell throw automatically on non-zero exit codes.
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        return $true    # signals failure to the parallel collector
    }
    ...
    if (Compare-Object $bbRefs $ghRefs) {
        & $fail 'REF MISMATCH'
    }
    ...
    git ls-remote "ssh://bitbucket/$Repo" | ForEach-Object {
        $parts = $_ -split '\s+', 2
        $sha   = $parts[0]
    ...
    if ($bbDefault -ne "refs/heads/$ghDefault") {
